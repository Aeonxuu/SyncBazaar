...
void main() {
  ...
  group('events', () {
    ...
    test('maps the venue from establishment, which the app calls a company', () {
    ...
    test('applies the fallback payment methods to every event', () {
      final events = mapEventsResponse(
        ...
        fallbackCustomMethods: const [
          BazaarPaymentMethod(name: 'GCASH', extraFieldLabel: 'Reference Number'),
        ],
      );

      expect(events.single.acceptedPaymentMethods, ['CASH', 'GCASH']);
      expect(events.single.customOtherMethods.single.extraFieldLabel,
          'Reference Number');
    });
  });
